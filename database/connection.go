...
func ConnectDb() {
	client, err := mongo.NewClient(options.Client().ApplyURI("mongodb://localhost:27017/ecommerce"))
	if err != nil {
	...
	if err != nil {
		log.Println("Unable to initialize database connectors:", err)
		return
	}
...
// 		uri = constant.MDBUri
// 	}
// 	client, err := mongo.NewClient(options.Client().ApplyURI(fmt.Sprintf("%s%s", "mongodb+srv://Diwakar:@Abcdef14#@cluster0.de77o86.mongodb.net/", uri)))
